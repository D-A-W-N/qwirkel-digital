...
**Erster echter Release-Zyklus (2026-08-02) fand einen zweiten Bug**: Nach dem Release von v0.5.0 (und den zwischenzeitlich von anderer Seite veröffentlichten v0.6.0/v0.7.0) schlug ein echter In-App-Update-Versuch fehl. Ursache: `Expand-Archive` verweigert grundsätzlich jede Datei, deren Name nicht buchstäblich auf `.zip` endet (`NotSupportedArchiveFileExtension`) — unabhängig vom tatsächlichen Dateiinhalt. Der gemeinsame Download-Helper `downloadAssetNextTo()` (in `update_applier.dart`, von allen drei Plattformen genutzt) benennt die heruntergeladene Datei aber immer `.qwirkle-update-download`, ganz ohne Endung — auf macOS/Linux unproblematisch (`ditto`/`tar` prüfen die Erweiterung nicht), unter Windows aber ein garantierter, 100% reproduzierbarer Fehlschlag bei jedem einzigen Update-Versuch. Sichtbare Spur des fehlgeschlagenen Versuchs: `.qwirkle-update-download` (voller, gültiger Download) blieb liegen, `.qwirkle-update-extract` existierte, aber leer.

Fix: In `WindowsUpdateApplier.prepare()` wird die heruntergeladene Datei vor dem Entpacken zu `<name>.zip` umbenannt; `cleanupStaleBackup()` räumt zusätzlich eine liegen gebliebene umbenannte Zip-Datei auf (Absturz zwischen Umbenennen und Löschen). Mit echtem Download von der GitHub-Release-CDN, echter Checksummen-Prüfung, echtem `Expand-Archive`-Aufruf und dem kompletten Copy-and-Relaunch-Zyklus end-to-end erneut verifiziert.
